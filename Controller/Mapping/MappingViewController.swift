...
//        setupDataForCollectionCell()
        ...
        view.addSubview(storeCardCollectionView)
...
//                self.storeCardCollectionView.reloadData()
    ...
    private func fetchData(isLogin _: Bool, competion: @escaping () -> Void) {
...
//            self.addChild(controller)
    ...
    func scrollViewWillEndDragging(_ scrollView: UIScrollView, withVelocity _: CGPoint, targetContentOffset: UnsafeMutablePointer<CGPoint>) {
        let itemSize = CGSize(width: storeCardCollectionView.frame.size.width - 2 * 16, height: storeCardCollectionView.frame.size.height - 2 * 6)
...
extension MappingViewController {
    ...
    @objc private func floatBtnAction(sender _: UIButton) {
